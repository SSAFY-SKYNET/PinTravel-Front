<template>
  <div class="relative group overflow-hidden rounded-lg cursor-pointer h-full w-full" @click="handleClick">
    ...
    <img
        :src="item.imageUrl"
        @load="loading = false"
        class="rounded-lg h-full w-full object-cover group-hover:brightness-90 transition-all"
        v-show="!loading"
    />
    <div
...
import {updateThumbnail} from "@/api/board.js";
...
const handleDeletePin = async () => {
  ...
  await updateThumbnail(props.boardId)
};
</script>
